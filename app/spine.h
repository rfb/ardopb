...
typedef enum {
	...
	APP_EV_FAULT,     /**< @c text: something the operator has to be told. */
} app_event_kind;
 ...
 *
 * The caller closes its audio backend *before* calling this and its PTT
 * *after* -- unkeying has to outlive the device that keyed.
 */
...
/**
 * @brief Report a device fault. **Modem thread only.**
 *
 ...
 * emits ::APP_EV_FAULT, and stops accepting transmit data.
 */
void app_report_fault(app_spine *sp, const char *what);

/** @brief True once ::app_report_fault has been called. **Modem thread only.** */
