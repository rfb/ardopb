...
| `modem` | `rxquality` — decode-quality metric | **in progress** — the constellation quality a receiver reports (rides in the ACK/NAK, drives gear-shift). the pure metrics are **done** — 4FSK (`Update4FSKConstellation`, preserving the plot-geometry accident, [analysis/12](../analysis/12-normative-accidents.md) #7) and PSK/QAM (`UpdatePhaseConstellation`), each proven bit-for-bit over 20 000 patterns. **wired** into the demod's deliver path, so `ardop_event.quality` is now populated; `make golden-core` checks it against ardopcf's recorded quality (14/17 exact, the rest off-by-one heuristic drift, reported not enforced as the inherited golden does) |
