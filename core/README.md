...
| `modem` | `rxquality` — decode-quality metric | **in progress** — the constellation quality a receiver reports (rides in the ACK/NAK, drives gear-shift). 4FSK done, proven bit-for-bit to `Update4FSKConstellation` over 20 000 patterns (preserving the plot-geometry accident, [analysis/12](../analysis/12-normative-accidents.md) #7); PSK/QAM (`UpdatePhaseConstellation`) next |
| **`modem`** | **core RX/TX complete** | modulate, goertzel, demodulate, busy, rxquality (4FSK) |
