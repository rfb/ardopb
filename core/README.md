...
| `link` | FSM + gear-shift | not started — the pure leaves above are the pieces it composes; `Gearshift_9` and the data-mode ladder are stateful and land with the FSM. See [analysis/02](../analysis/02-protocol-fsm.md) (the ~60-transition table) and [analysis/10](../analysis/10-modem-link-design.md) (the event/action contract) |
